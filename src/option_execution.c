...
Options *parse_options(Dictionary *conf, int argc, char **argv) {
  ...
  opts->dest_host = (char *)dictionary_get(
      ((Dictionary *)dictionary_get(conf, "hosts")), "default");
  int c;
  while (1) {
    ...
    case 'h':
      opts->dest_host = (char *)dictionary_get(
          ((Dictionary *)dictionary_get(conf, "hosts")), optarg);
      break;
    case 't':
      opts->transfer = 1;
      break;
  ...
  _debug(printf("[D] source file: %s\n", opts->source_file);) _debug(
      printf("[D] destination host: %s\n", opts->dest_host);) return opts;
}
